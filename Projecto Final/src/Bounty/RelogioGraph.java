...
public class RelogioGraph extends JFrame {
	...
	public RelogioGraph() {
		...
		Image image = Toolkit.getDefaultToolkit()
				.getImage(getClass().getResource("/Bounty/resources/iconrelogio2.jpg"));
		setIconImage(image);
		setResizable(false);
		...
		/*
		 * Desenha Elementos Fixos e Adiciona Elementos que vão actualizar
		 */
		...
		ActionListener updateClock = new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				relogioDigitalFuso.setText(
						tempo.getRelogioFuso(fusoSelector.getSelectedItem().toString(), FusosHorarios.fusosHorarios));
			}
		};
		// Temporizador 1seg
	...
	public void desenhaPainelTarefas() {
		...
		btnSave.setIcon(
				new ImageIcon(RelogioGraph.class.getResource("/javax/swing/plaf/metal/icons/ocean/floppy.gif")));
		...
		btnLoad.setIcon(new ImageIcon(
				RelogioGraph.class.getResource("/com/sun/javafx/scene/control/skin/modena/HTMLEditor-Outdent.png")));
		...
		// Adiciona Texto Formatado no meth fillTarefasDone() -> para tarefas
		// Concluidas
		...
		chbxValidar1.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				// copia texto do JTextField textTarefa1 para o ArrayList das
				// Tarefas Concluidas
		...
		// Actualiza as tarefas concluidas quando o ratoe sai da area das
		// tarefas "por fazer"
	...
	public void desenhaElementos() {
		...
		// Chama o metodo que adiciona o Botao Sobre -> Na minha opinião é um
		// desenho por isso está aqui :)
		adicionaBotaoSobre();
	...
	public JPanel desenhaPonteirosFuso(JComboBox fusoSelector) {
		JPanel ponteirosfuso = new JPanel() {
			...
			protected void paintComponent(Graphics relogioAnalog) {
				...
				/*
				 * Função getCoord Ponteiros.getCoordPonteiro(graus, raio,
				 * centroX, centroY)
				 */

				/* draw Horas */
				grau = Ponteiros.getGrausFromHoraFuso(fusoSelector.getSelectedItem().toString(),
						FusosHorarios.fusosHorarios);

				g2d.drawLine(50, 50, Ponteiros.getCoordXfromGraus(grau, 20, 50),
						Ponteiros.getCoordYfromGraus(grau, 25, -50));

				/* draw Minutos */
				grau = Ponteiros.getGrausFromMinuto();
				g2d.drawLine(50, 50, Ponteiros.getCoordXfromGraus(grau, 35, 50),
						(Ponteiros.getCoordYfromGraus(grau, 35, -50)));

				/* draw Segundos */
				grau = Ponteiros.getGrausFromSegundo();
				g2d.drawLine(50, 50, Ponteiros.getCoordXfromGraus(grau, 40, 50),
						(Ponteiros.getCoordYfromGraus(grau, 40, -50)));
